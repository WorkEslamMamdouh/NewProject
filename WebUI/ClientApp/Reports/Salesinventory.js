...
(function (Salesinventory) {
    ...
    function Display() {
        ...
        Ajax.Callsync({
            ...
            success: function (d) {
                var result = d;
                if (result.IsSuccess) {
                    Get_IQ_The_Gard = result.Response;
                    InitializeGrid();
    ...
    function _SearchBox_Change() {
        ...
        if (searchbutmemreport.value != "") {
            ...
            SearchDetails = Get_IQ_The_Gard.filter(function (x) { return x.id_Num.toString().search(search_1) >= 0 || x.PRODUCT_NAME.toLowerCase().search(search_1) >= 0 || x.Shift_User.toLowerCase().search(search_1) >= 0; } /*|| x.PortName.toLowerCase().search(search) >= 0*/);
